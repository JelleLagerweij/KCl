...
# Parsa Data
folder = ['../Parsa/36/', '../Parsa/76/']
m_real_P = np.zeros(len(m))
E_Ons_P = np.zeros(len(m))
E_Ons_P_err = np.zeros(len(m))

E_NE_P = np.zeros(len(m))
E_NE_P_err = np.zeros(len(m))
...
# plt.errorbar(m_V, E_Ons_V, yerr=E_Ons_V_err, color=cm(2), fmt='o', capsize=11,
#               elinewidth=2, capthick=2, markersize=9, label=r'GK $\sigma$')
# plt.scatter(m_V, E_NE_V, color=cm(3), marker='>', s=64, label=r'GK $\sigma_{NE+YH}$')
...
# plt.savefig('Delft_Madrid_KCl.pdf', bbox_inches='tight')
...
plt.savefig('ScalingKCl4.pdf', bbox_inches='tight')